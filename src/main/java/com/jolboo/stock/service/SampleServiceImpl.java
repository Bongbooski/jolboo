...
public class SampleServiceImpl implements SampleService {
  ...
  public void setAccessToken() {
    ...
    if (byIssuedDate.isEmpty()) {
      AccessTokenResponseDTO client_credentials =
          stockOauthFeign.getAccessToken(
      ...
      OauthVO oauth =
          OauthVO.builder()
              .issuedDate(today)
              .accessToken(
                  client_credentials.getToken_type() + " " + client_credentials.getAccess_token())
              .expiredDatetime(
                  OffsetDateTime.parse(
                      client_credentials.getAccess_token_token_expired(),
  ...
  public VtsGetBalanceResponseDTO getVtsGetBalance() {
    ...
    VtsGetBalanceRequestParamDTO requestParamDTO =
        VtsGetBalanceRequestParamDTO.builder()
            .CANO(cano)
            .ACNT_PRDT_CD("01")
            .AFHR_FLPR_YN("N")
            .OFL_YN("")
            ...
            .CTX_AREA_FK100("")
            .build();
    VtsGetBalanceResponseDTO result =
        stockFeign.getVtsBalance(
            requestParamDTO.getCANO(),
            requestParamDTO.getACNT_PRDT_CD(),
            requestParamDTO.getAFHR_FLPR_YN(),
            requestParamDTO.getOFL_YN(),
            requestParamDTO.getINQR_DVSN(),
            requestParamDTO.getUNPR_DVSN(),
            requestParamDTO.getFUND_STTL_ICLD_YN(),
            requestParamDTO.getFNCG_AMT_AUTO_RDPT_YN(),
            requestParamDTO.getPRCS_DVSN(),
            requestParamDTO.getCTX_AREA_FK100(),
